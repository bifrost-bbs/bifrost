    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!(
                    "      --crawl, --auto-navigate Automate navigating BBS options at random"
                );
                println!(
                    "  -n, --crawl-steps <STEPS>  Number of crawl steps to execute (default: 50)"
                );
            ...
            if !submit_indices.is_empty() {
                ...
                for &idx in &submit_indices {
                    ...
                    if is_exit_button && has_forward_options {
                        ...
                        let unvisited_forward_count = submit_indices
                            .iter()
                            .filter(|&&i| {
                    ...
                    if let (Some(_last_fid), Some(ref last_bid)) =
                        (self.last_form_id, &self.last_button_id)
                    {
                        if last_bid == "back"
                            && self
                                .visit_counts
                ...
                *self
                    .visit_counts
                    .entry((form.form_id, chosen_field.id.clone()))
                    .or_insert(0) += 1;
...
async fn main() -> Result<()> {
    ...
    println!(
        "Connecting to virtual radio transport at {}...",
        config.server_addr
    );
    ...
    if config.auto_crawl {
        ...
        tokio::spawn(async move {
            log::info!(
                "[AUTO-CRAWLER] Started automated navigation ({} steps, {}ms delay)",
                steps_total,
                delay_ms
            );
            if is_headless {
            ...
            while step_count < steps_total || steps_total == 0 {
                ...
                if is_headless {
                    println!(
                        "[AUTO-CRAWLER] Step {}/{}: {}",
                        step_count, steps_total, action.description
                    );
                }
                                        ...
                                        if f.is_submit {
                                            ...
                                            let id_first =
                                                f.id.chars()
                                                    .next()
                                                    .map(|ch| ch.to_ascii_lowercase());
                                            if label_first == Some(lower_c)
                                                || id_first == Some(lower_c)
                                            {
                                                matched_idx = Some(i);
                                                break;
                                            }
        ...
        .await
        {
            Ok(Ok(packet)) => {
                if !packet.is_broadcast
                    && packet.dst_node != [0; 32]
                    && packet.dst_node != client_key
                {
                    continue;
                }
                    ...
                    for _ in 0..param_count {
                        if cur < payload.len() {
                            ...
                            if cur + p_len <= payload.len() {
                                let s = String::from_utf8_lossy(&payload[cur..cur + p_len])
                                    .into_owned();
                                params.push(s);
                                cur += p_len;
                            }
            ...
            0xC8 => {
                ...
                if i + 6 < payload.len() {
                    ...
                    if let Some(menu_csv) = get_client_asset_content(server_node_id, id) {
                        ...
                        if let Some(fg) = menu_def.field_fg {
                            form_state.field_fg = fg;
                        }
                        if let Some(bg) = menu_def.field_bg {
                            form_state.field_bg = bg;
                        }
                        ...
                        for (btn, btn_width, base_row) in enabled_buttons {
                            ...
                            apply_color_attribute(
                                (form_state.submit_bg << 4) | form_state.submit_fg,
                            );
                            print!(
                                "\x1b[{};{}H[ {} ]\x1b[0m",
...
fn save_asset_to_cache(node_id: &[u8; 32], asset_id: u16, data: &[u8]) {
    ...
    let ext = if asset_id == 0x00DF {
        "bin"
    } else if data.starts_with(b"# form_id=")
        || (data.len() > 10
            && data[..data.len().min(100)]
                .windows(13)
                .any(|w| w == b"# tag,id,label"))
    {
        "csv"
...
fn get_client_asset_content(node_id: &[u8; 32], asset_id: u16) -> Option<String> {
    ...
    for ext in &exts {
        ...
        let root_file =
            find_workspace_path(".client_cache").join(format!("{:04x}.{}", asset_id, ext));
...
mod tests {
    ...
    fn test_render_menu_bytecode_parsing() {
        ...
        assert!(
            menu_asset.is_some(),
            "main_nav.csv asset 0x0103 should resolve"
        );
        ...
        assert_eq!(
            form.fields.len(),
            4,
            "All 4 buttons in main_nav should be populated"
        );
    ...
    fn test_render_menu_bottom_aligned_stays_in_virtual_terminal() {
        ...
        for f in &form.fields {
            ...
            assert!(
                f.row <= 25,
                "Field row {} should be <= 25 inside virtual terminal",
                f.row
            );
            assert!(
                f.col + f.width <= 80,
                "Field col + width {} should not overflow 80 cols",
                f.col + f.width
            );
        }
    }
