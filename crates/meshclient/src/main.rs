...
async fn main() -> Result<()> {
    ...
    let input_handle = tokio::spawn(async move {
        while let Some(KeyEvent { code, modifiers, kind, .. }) = key_rx.recv().await {
...
fn get_viewport_offsets(layout: LayoutMode, term_width: u16, term_height: u16) -> (u16, u16, u16, u16) {
    let (w, h) = match layout {
        LayoutMode::Full => (80, 25),
        LayoutMode::Compact => (40, 25),
    };
    let col_offset = if term_width > w { (term_width - w) / 2 } else { 0 };
    let row_offset = if term_height > h { (term_height - h) / 2 } else { 0 };
...
fn interpret_bytecode(payload: &[u8], form_state: &mut FormState, col_offset: u16, row_offset: u16) {
    let mut i = 0;
    while i < payload.len() {
        let op = payload[i];
        match op {
            ...
            0x01 => {
                ...
                print!("\x1b[2J\x1b[H");
                let (term_w, term_h) = crossterm::terminal::size().unwrap_or((80, 25));
                let (_, _, w, h) = get_viewport_offsets(if col_offset > 25 { LayoutMode::Full } else { LayoutMode::Compact }, term_w, term_h);
                draw_viewport_border(col_offset, row_offset, w, h);
                print!("\x1b[{};{}H", row_offset + 1, col_offset + 1);
            ...
            0xC3 => {
                ...
                if i + 2 < payload.len() {
                    let col = payload[i + 1];
                    let row = payload[i + 2];
                    print!("\x1b[{};{}H", row_offset + row as u16 + 1, col_offset + col as u16 + 1);
                    i += 3;
                } else {
                    i += 1;
                }
                ...
                if i + 5 < payload.len() {
                    ...
                    if i + 5 + id_len < payload.len() {
                        ...
                        if val_len_idx + 1 + val_len <= payload.len() {
                            ...
                            print!("\x1b[{};{}H{:width$}\x1b[0m", row_offset + row as u16 + 1, col_offset + col as u16 + 1, val, width = width as usize);
            ...
            0xD2 => {
                ...
                if i + 4 < payload.len() {
                    ...
                    if i + 4 + id_len <= payload.len() {
                        ...
                        print!("\x1b[{};{}H[ {} ]\x1b[0m", row_offset + row as u16 + 1, col_offset + col as u16 + 1, id);
                        i = i + 4 + id_len;
                    } else {
                        i += 1;
                    }
...
fn render_cached_asset(asset_id: u16, col_offset: u16, row_offset: u16) {
    ...
    if let Some(p) = path {
        if let Ok(content) = std::fs::read_to_string(p) {
            ...
            let aligned_content = content.replace("\r\n", "\n").replace("\n", &newline_replacement);
